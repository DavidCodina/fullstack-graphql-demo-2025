...
export type ContextTypeUser = Omit<UserType, 'password' | 'token'>
...
export const createContext = async (
  ...
  let user: Omit<UserType, 'password' | 'token'> | null = null
  ...
  try {
    ...
    user = await User.findById(userId).select('-password -token').lean().exec()
